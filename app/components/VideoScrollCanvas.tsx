...
export default function VideoScrollCanvas({ onReady }: { onReady?: () => void } = {}) {
  ...
  // Notify parent as soon as the hero VIDEO is ready — don't wait for all 422 frames.
  // Frames load progressively in the background; canvas draws whatever is available.
  useEffect(() => {
    if (videoLoaded && !readyCalledRef.current && onReady) {
  ...
  useEffect(() => {
    ...
    const preloadImages = () => {
      let loadedCount = 0;
      // Eager-load first 30 frames so scrolling feels instant; rest load in background.
      const EAGER = 30;

      for (let i = 1; i <= TOTAL_FRAMES; i++) {
        const img = new Image();
        // fetchpriority lets browsers prioritise the first batch
        if (i <= EAGER) (img as HTMLImageElement & { fetchPriority: string }).fetchPriority = 'high';
        ...
        img.onload = () => {
          ...
          if (loadedCount === 1) {
            ...
            resizeCanvas();
            setImagesLoaded(true); // show canvas as soon as frame 1 is ready
          }
        };
    ...
    <div ref={containerRef} className="relative h-[540vh] w-full bg-gradient-to-br from-slate-900 to-slate-950" style={{ minHeight: '100vh' }}>
      <div className="sticky top-0 h-screen w-full flex items-center justify-center">
        <div className="relative w-full h-full overflow-hidden">
          ...
          {/* First frame as poster — zero-flash before canvas is ready */}
          {!imagesLoaded && (
            <img
