...
function getResponsive(
  ...
  if (w < 480)  return { cols: 2, ghostCols: 0, cellH: 130 };
  if (w < 768)  return { cols: 3, ghostCols: 0, cellH: 150 };
  if (w < 1024) return { cols: 5, ghostCols: 0, cellH: 180 };
  return { cols: desktopCols, ghostCols: desktopGhostCols, cellH: 160 };
}
...
function GhostCell({ cellH }: { cellH: number }) {
  return <div style={{ height: cellH }} suppressHydrationWarning />;
}
...
function BrandCell({
  ...
  return (
    <motion.div
      ...
      style={{ height: cellH }}
    >
      {/* Fill spotlight - only render after client hydration */}
      {mounted && (
        <div
      ...
      {/* Logo / text */}
      <div className="relative z-10 flex items-center justify-center w-full h-12 sm:h-16 md:h-20">
        {brand.logo ? (
